...
并发：指一个系统能够同时处理多个任务的能力，但**在任意时刻可能只有一个任务在执行。操作系统通过快速切换任务来实现这种效果，使得每个任务都似乎在同时运行**。这适用于单核处理器或多核处理器。

并行：指多个任务真正地同时执行，通常发生在**多核**处理器上，每个核心可以独立执行一个任务。并行需要硬件支持，例如多核CPU或GPU。


:::info 举例
单厨师 vs. 多厨师做饭

并发: 就像一个厨师（单核CPU）要准备一道包含炒菜、煮汤和蒸米饭的套餐。他可以先开始煮水烧开用来蒸米饭，然后利用水烧开的这段时间去切菜、热锅炒菜，炒好菜后再去看汤煮得怎么样。厨师在不同的任务之间切换，没有完全空闲，提高了效率。这就是并发——任务交替进行，共享一个资源（厨师）。

并行: 如果有三个厨师（多核CPU），他们可以分别同时负责炒菜、煮汤和蒸米饭。这三个任务在同一个时刻都在被执行。这就是并行——任务真正同时进行，每个任务占用独立的资源（厨师）。
:::
## 常见兼容问题解决
