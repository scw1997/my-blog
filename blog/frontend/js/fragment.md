...
- defer和async属性介绍详见：[浏览器渲染机制 > HTML解析](/frontend/advanced/browser-render.html#html解析)
...
- js函数中的传参都是`按值传递`的，在函数中修改传参值是修改的是局部变量，不影响函数外部变量。但如果传递的是引用类型值并在函数中修改了其内部(比如push或添加属性)，则函数外步也会反映该变化。因为此时这种操作访问的是外部作用域的堆内存中的对象引用副本.

:::code-group 
  ```js [示例1]
  const func = (num) => {
    num = 1; //num是函数参数，函数内部修改num的值不会影响函数外部的num变量
  };
  let a = 2
  func(a)
  console.log('a',a) //2
