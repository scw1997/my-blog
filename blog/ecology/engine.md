...
:::info vite生产环境为何仍需打包？打包工具又为何使用rollup而不是esbuild?

A1：生产环境的核心要求是`性能优化和稳定性`，这包括代码压缩分割，资源拆分，平台兼容性处理等。然而开发环境的核心要求是`快速反馈`即可。若直接使用原生ES Module资源部署生产环境，则无法保证代码性能和稳定性。
