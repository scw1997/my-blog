...
||Redux| Mobx              |
|--|--|-------------------|
|设计思想	| 函数式编程	| 对象编程和响应式编程        |
|状态管理|	集中管理在单一store中| 	可按模块划分多个独立的store |
|数据可变性	|强调不可变性，通过返回新状态对象来更新状态	| 可直接使用新值更新状态对象     |
|状态更新方式|	使用dispatch分发action，通过reducer更新state|	自动追踪依赖并更新|
|适用场景|适合需要严格控制数据流和状态更新的大型应用|适合需要快速开发和细粒度状态管理的应用|
...
- hook的useState执行set时，如果使用函数写法即`setState(prev=>...)`,那么这里拿到的prev一定是最新的state
- 一道测试题
    ```jsx
