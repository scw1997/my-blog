...
- **定义**：进程是操作系统分配资源的基本单位，是程序正在运行的一个实例。
- **特点**：
...
## 线程
- **定义**：线程是 CPU 调度的基本单位，是进程中可独立执行的最小单位。
- **特点**：
...
多线程的执行调度是随机的，比如正在执行A线程时也可能会在执行B线程。如果涉及访问或修改同一变量时，可能会产生数据不一致问题。
...
| 特性  | 描述 | 实现方式                                                                         |
|:----| :--- |:-----------------------------------------------------------------------------|
| 原子性 | 操作的完整性，确保一个操作（或一组操作）要么全做，要么全不做，不能被打断。 | `synchronized`, `AtomicInteger`, `ReentrantLock`                             |
| 可见性 | 当一个线程修改了共享变量，其他线程能立即感知到这个修改。 | `volatile`, `synchronized`, `final`, `ReentrantLock`                         |
| 有序性 | 程序执行的顺序按照代码的先后顺序执行 | `volatile`, `synchronized`, `final`, `ReentrantLock`，`Thread.join()或start()` |

:::warning
- `对于复合操作（如 i++这种包含了对值读取，修改并写回的多个步骤）并不能保证原子性`，它只能保证单次读/写的原子性（对于基本类型和引用类型）
- 这三个特性同时得以保障才能够确保一个线程能够“正确”地看到(即看到的不是“半成品”)其他线程对共享变量所做的更新并按照预期执行。`它们共同构成了线程安全的基础`。
:::
...
锁能够保护共享数据以实现线程安全,其作用包括**保障原子性、可见性和有序性**。




#### （1）内部锁（synchronized关键字）
...
// 代码块级
//这里lockObject为锁对象。
// 绝大部分情况下锁对象必须保证唯一（即所有线程加锁的逻辑都使用这唯一的锁对象，任何人来执行都需要获取这个唯一锁才有权限访问。如果各自用的锁对象不一样，锁就无意义。）
// 小部分情况需要用到多个锁，如不同用户ID的锁对象不同（同一个用户访问用同一个锁，不同用户用不同锁）
...
**思想**：无锁读写，提交时检查是否被别人修改过（即认为线程安全问题发生概率低）
...
Java虚拟机（JVM）本质是一个运行在计算机上的程序，负责加载、验证、执行 Java 字节码（bytecode）文件，并管理内存、线程、安全等底层资源。

|名词|介绍|
  |--|--|
|JDK|JDK是java开发工具包，包含了`JRE + 开发工具集`（如编译器 javac、调试器 jdb、打包工具 jar 等）。用于编写代码+运行程序。|
|JRE|JRE给普通用户运行 Java 程序用的环境。包含了`JVM + Java 核心类库`。如果只需要运行class文件或jar 文件，则只需要JRE即可。|
|JVM|JVM是虚拟机，负责`加载编译好的 .class 字节码文件，并将其翻译成当前操作系统能识别的机器语言`。同时，它还负责`内存分配,垃圾回收（GC）等底层工作和即时编译（提升执行效率）`。|

#### 虚拟机的作用

- `跨平台运行`：Java 源代码（.java）通过调用**javac java文件名**被编译成与平台无关的**字节码（.class 文件）**，由 JVM通过**java class文件名**在不同操作系统（Windows、Linux、macOS 等）上解释或编译执行。
- `自动内存管理`：JVM 提供**垃圾回收（GC）机制**，自动回收不再使用的对象，避免内存泄漏和手动内存管理的复杂性。
- `安全性`：JVM 提供字节码校验、类加载器隔离、安全管理器等机制，防止恶意代码破坏系统。
- `性能优化`:通过 **即时编译器**（JIT, Just-In-Time Compiler）将热点代码编译为本地机器码，提升运行效率。
...
- `启动类加载器`：负责加载 Java **最核心**的类库，如 java.lang.* 类。这些类通常存放在 <JAVA_HOME>/jre/lib 目录下。由于它不是Java类，所以在Java代码中无法直接获取它的引用。
- `扩展类加载器`：启动类加载器的子加载器。负责加载Java的**扩展类库**，这些类通常存放在 <JAVA_HOME>/jre/lib/ext 目录下。
- `应用程序类加载器`：扩展类加载器的子加载器。负责加载用户**自己编写的类以及项目依赖的第三方库**，通常存放在 当前项目的 classpath（.class文件） 中。
- `自定义类加载器`：自定义类加载器，可以继承 java.lang.ClassLoader 类，重写findClass方法。实现自己的类加载逻辑。
:::warning 注意
- 在JDK9之后，扩展类加载器被替换成了`平台类加载器`
:::

#### 1.1 双亲委派模型

![双亲委派模型.png](/class-loader.png)
类加载器遵循一套被称为“双亲委派”（Parent Delegation Model）的核心规则来协同工作。

1. 当一个类加载器（比如应用程序类加载器）收到加载某个类的请求时，它不会立刻自己动手。 
2. 它会先判断这个类是否已经加载过。是则直接返回；否则将这个请求向上委派给自己的父加载器（扩展类加载器）。
3. 扩展类加载器收到请求后，同样不会自己加载，而是同理步骤2继续向上委派给它的父加载器（启动类加载器）。
4. 最终，请求会到达最顶层的启动类加载器。
5. 启动类加载器会检查自己是否能加载这个类（即是否在自己的核心类库路径下）。
6. 如果能加载：则直接返回加载好的类，任务结束。
7. 如果不能加载：则将加载失败的结果逐层返回给子加载器。
8. 子加载器收到父加载器无法加载的通知后，才会尝试自己加载。

:::tip 双亲委派特点
优点：
- **保证安全性**：防止Java核心类库被篡改。例如，即使你恶意编写了一个自己的 java.lang.String 类，由于双亲委派机制的存在，这个请求最终会到达启动类加载器，由它加载官方标准的 String 类。你自定义的类永远不会被加载，从而保证了系统的安全。
- **避免重复加载**：当一个类已经被父加载器加载后，子加载器就不会再去重复加载它，确保了类在JVM中的唯一性，节省了内存资源。

缺点：
- **无法实现“父类调用子类”**：双亲委派的加载链路是单向向上的，这意味着父加载器加载的类是全局共享的，但子加载器加载的类，父加载器是看不到的。
- **类加载隔离性差，易引发版本冲突**：双亲委派模型追求类的全局唯一性。如果你的项目中，不同的框架依赖了同一个第三方库（比如 Guava）的不同版本，按照双亲委派机制，全局只会加载其中一个版本。
- **不支持热部署与热替换**：在双亲委派模型下，一个类一旦被某个类加载器加载，就不会被重新加载。这在需要动态修改代码并生效的场景（如 Web 应用的热部署）中是无法接受的，通常必须重启整个 JVM。
- **不适合模块化与容器化场景**：在 Web 容器（如 Tomcat）或模块化系统（如 OSGi）中，往往需要在同一个 JVM 中运行多个独立的应用或插件。如果所有应用共享同一个类加载空间，会导致类冲突、配置污染，且应用无法被独立卸载。
:::

#### 1.2 打破双亲委派模型

双亲委派的逻辑是写在 java.lang.ClassLoader 类的 loadClass() 方法里的。因此，只要我们`自定义一个类加载器，并重写 loadClass() 方法`，改变它默认“先向上委托父加载器”的逻辑，就能打破这个机制。

:::tip 示例：Tomcat 容器

一个 Tomcat 容器下通常会部署多个 Web 应用（WAR包）。如果应用 A 依赖 Spring 4，应用 B 依赖 Spring 5，按照双亲委派，全局只能加载一个版本的 Spring，必然导致冲突。

Tomcat 自定义了 WebAppClassLoader。它改变了加载顺序，优先加载自己应用目录（如 WEB-INF/classes 和 WEB-INF/lib）下的类，加载不到再委派给父加载器。这样每个 Web 应用都有自己独立的类加载空间，实现了类隔离和版本共存。
:::
...
- **方法区**：存储`class类结构信息（元信息）、常量、静态变量、即时编译器编译后的代码(.class文件)`

- **堆**： 存放几乎所有`对象实例和数组`

:::tip 元空间/永久代

在 JDK 7 及以前，方法区是由`永久代`实现的，它和 Java 堆内存连在一起，大小固定。

JDK 8 之后，这个区域改为由`元空间`实现，目的是解决两个问题：

- 元空间使用的是`操作系统内存`（永久代是JVM的堆内存，内存上限有限，很容易溢出），只要不超上限，可持续分配。
- 优化垃圾回收的策略：永久代在堆上，垃圾回收机制一般使用`老年代`的垃圾回收方式，不够灵活。使用元空间 之后单独设计了一套适合方法区的垃圾回收机制。
...
- **虚拟机栈**：每个线程一个栈，存储栈帧（Stack Frame），包含局部变量表、操作数栈、动态链接、方法返回地址等。每个方法调用创建一个栈帧，`方法调用 = 压栈，方法返回 = 出栈`。
...
负责执行字节码指令，由`解释器, 即时编译器，垃圾回收器`构成。

#### 3.1 垃圾回收器

垃圾回收器核心职责是自动找出并清理掉那些不再被使用的对象，释放内存空间，从而让开发者从繁琐的手动内存管理中解脱出来，有效避免内存泄漏。

在清理垃圾之前，GC首先需要判断哪些对象是垃圾。

##### 3.1.1 如何判断对象是垃圾（对象是否被引用）？

传统方式`引用计数法`会为每个对象维护一个引用计数器，当对象被引用时加1，取消引用时减 1，`存在循环引用问题`所以Java没有使用这种方法。

目前主流的算法是`可达性分析`。

它的原理是：从一组被称为 GC Roots 的根对象出发，沿着引用链向下搜索。如果一个对象从任何一个 GC Root 都不可达（即没有任何强引用指向它），那么它就被判定为可回收的垃圾。
...
确定了哪些是垃圾后，JVM 采用了不同的清理策略：

| 算法名称  | 核心原理                                       | 优点 | 缺点 | 典型应用场景 |
|:------|:-------------------------------------------| :--- | :--- | :--- |
| 标记-清除 | 从 GC Roots 遍历并标记存活对象，然后统一清除所有未标记的死亡对象。     | 原理简单，不需要移动存活对象。 | 会产生大量不连续的内存碎片，影响后续大对象的分配。 | 老年代回收（如早期的 CMS 收集器）。 |
| 复制算法  | 将内存分为两块，每次只用一块。GC 时将存活对象复制到另一块，然后一次性清空当前块。 | 内存连续规整，无内存碎片；分配和回收效率极高。 | 内存利用率低（浪费一半空间）；存活对象多时复制开销大。 | 新生代回收（绝大多数现代收集器）。 |
| 标记-整理 | 先标记存活对象，然后将它们向内存的一端移动，最后清理边界外的所有空间。        | 避免了内存碎片，内存利用率高。 | 移动存活对象并更新引用的成本较高，会增加停顿时间。 | 老年代回收（如 Parallel Old、Serial Old）。 |
| `分代收集` | **结合上述算法，根据对象存活周期将内存分为新生代和老年代，采用不同算法。**    | 扬长避短，综合效率最高。 | 机制复杂，需要维护对象年龄等额外信息。 | 现代商业虚拟机的主流策略（如 G1）。 |

现代商业虚拟机（如 HotSpot）普遍采用`分代收集`算法。将堆内存划分为两个主要区域，并为它们量身定制了不同的回收策略：

- `新生代`：专门用来存放新创建的对象。
- `老年代`：存放那些在新生代中经历过多次垃圾回收后，依然存活的“老顽固”对象。

#### 4.本地方法接口/库

- **本地方法接口**：允许 Java 代码调用本地（如 C/C++）库，扩展 JVM 功能（如访问硬件、调用系统 API）。
- **本地方法库**：包含 JVM 使用的本地库（如 java.lang.System 的部分实现）。
...
#### 3. **类加载**：
   ...
   JVM负责将.class 文件加载到内存，并生成对应的 Class 对象。

   一个Java类从被加载到虚拟机内存中开始，到最终卸载出内存为止，它的整个生命周期会经历 以下三个阶段：
   ...
   | 初始化（Initialization） | 真正执行代码的阶段。此阶段会执行 `<clinit>` 方法：<br>- `static` 变量的显式赋值<br>- `static {}` 代码块<br>（按代码顺序执行）                                                      |
  > 当类被初始化后，就进入了**使用**阶段，供程序正常调用。当这个类的所有实例都被垃圾回收，且加载它的类加载器也被回收，同时它的 Class 对象没有在任何地方被引用时，该类就会进入**卸载**阶段，彻底从内存中清除。
  :::tip 类加载的触发时机？
  类并不会一上来就触发类加载，只有在“首次主动使用”时才会触发。
  以下操作会`触发类加载`：
  - 创建类实例（new）
  - 调用静态方法
  - 访问静态字段（非 final 常量）
  - 使用反射对类进行调用时（Class.forName()）
  - 启动类（含 main 方法的类）
...
- 发生区域：堆（Heap），用于存放几乎所有的对象实例。
- 典型场景：
  - 一次性从数据库查询出海量数据并全部加载到内存中。
  - 使用一个静态的 List 或 Map 作为缓存，但只添加数据，从不清理。
  - 存在`内存泄漏`，对象不再使用但引用未被释放。例如打开了数据库连接、网络套接字（Socket）、IO流等资源后，没有使用 try-with-resources 语法来显式关闭它们或者`ThreadLocal` 变量在使用后没有调用 remove() 方法清理。

**栈内存溢出**

- 错误信息：**java.lang.StackOverflowError、java.lang.OutOfMemoryError**
- 发生区域：虚拟机栈（VM Stack），每个线程私有，用于存储方法调用的栈帧。
- 典型场景：无终止条件的递归调用，创建的线程数量过多

**元空间溢出**

- 错误信息：**java.lang.OutOfMemoryError: Metaspace**
- 发生区域：元空间（方法区），用于存储类的元数据（如类结构、方法信息等）。
- 根本原因：程序在运行时动态生成了大量的类，耗尽了元空间。
- 典型场景： 大量使用 CGLib、ASM 等字节码增强框架动态生成代理类；基于OSGi等框架的热部署应用，频繁加载和卸载类，但旧的类加载器未被回收。

> 注意：因为程序计数器存储的当前执行的字节码的地址是一个固定长度的内存地址，`程序计数器是不会发生内存溢出的`。
:::
#### 6.  **程序终止**
...
| 引用类型 | 强度 | 回收时机                  | 核心场景                                               |
| :--- | :--- |:----------------------|:---------------------------------------------------|
| 强引用 | 最强 | 永不回收（哪怕内存溢出）          | 日常开发中普通的局部变量、成员变量、静态集合（如 ArrayList、HashMap）中存储的对象。 |
| 软引用 | 较强 | 内存不足、将要内存溢出时回收        | 内存敏感的高速缓存 。如缓存一些大图片或网页                             |
| 弱引用 | 较弱 | 只要发生垃圾回收就回收           | `ThreadLocal`、`WeakHashMap`                        |
| 虚引用 | 最弱 | 随时回收，`get()` 永远为 null | 跟踪对象回收，管理堆外内存                                      |

:::warning threadLocal为什么使用弱引用?
在 ThreadLocal 的底层，每个线程都有一个 ThreadLocalMap，里面存放着一个个 `Entry`（键值对）。
这个 Entry 的设计非常特殊：
- **Key**（键）：是 ThreadLocal 实例本身，并且被设计为弱引用（WeakReference）。
- **Value**（值）：是我们通过 threadLocal.set() 存入的数据，是强引用。

当ThreadLocal对象不再使用时（设置为null），为了防止对象本身发生内存泄露，使用弱引用可以让对象被回收。

但弱引用没有完全解决对象回收的问题。存入的Value值是强引用，无法被回收。合理做法是依然需要我们`手动调用 remove()`来切断强引用链。
:::


### 性能优化

JVM的性能优化通常伴随着JVM的参数设置，常见修改方式：

:::code-group
```bash [1.命令行指定（最常用）]
# 在启动 Java 程序时，直接在 java 命令后面跟上参数。这种方式最灵活，适合临时测试或简单的脚本启动。

# 设置初始堆内存为 2G，最大堆内存为 4G，并使用 G1 垃圾回收器
java -Xms2g -Xmx4g -XX:+UseG1GC -jar your-application.jar
```
```bash [2.使用参数文件]
# 如果你的启动参数非常多，全写在命令行里会显得极其臃肿。
# 你可以把参数写在一个文本文件里（比如 jvm.config），然后在启动时通过 @ 符号引用它。

# 第一步：创建jvm.config文件，其内容如下：
-Xms2g
-Xmx4g
-XX:+UseG1GC
-XX:MaxGCPauseMillis=200

# 第二步：在启动 Java 程序时，引用该参数文件
java @jvm.config -jar [应用名].jar
```
:::
...
| **元空间（方法区）溢出**      | `-XX:MaxMetaspaceSize=256m`（避免无限增长），JDK8之前使用 `-XX:MaxPermSize`   |
...
```bash
# 将初始堆内存（-Xms）和最大堆内存（-Xmx）设置为相同的值
# 此举可避免 JVM 在运行过程中频繁向操作系统申请和释放内存，减少性能抖动。
...
# 调整新生代与老年代占比(-XX:NewRatio)
# 如果你的应用会频繁创建大量生命周期很短的对象，可以适当增大新生代的比例，让对象在新生代就被回收，避免过早进入老年代。
-XX:NewRatio=2  # 老年代:新生代 = 2:1


# 使用 -XX:MetaspaceSize 和 -XX:MaxMetaspaceSize 来限制元空间的大小
# 此举可防止动态生成的类（如 CGLIB 代理）耗尽物理内存。
...
- **避免频繁创建对象**：在循环或高频调用的方法中，尽量复用对象（如ThreadLocal），或者合理使用对象池。
- **避免大对象分配**：避免一次性 new 大数组，改用流式处理。
- **避免字符串拼接**：用 `StringBuilder` 代替 `+`（尤其在循环中）。
- **集合初始化容量**：例如使用`new ArrayList(1000)` 避免多次扩容。
...
|--------|---------|---------|
| 客户端要求 | 需安装专用软件 | 只需浏览器 |
| 维护成本 | 高（需逐个更新客户端） | 低（服务器端更新即可） |
| 跨平台性 | 较差（需开发不同版本） | 好（浏览器通用） |
| 响应速度 | 快（部分逻辑本地处理） | 相对较慢（依赖网络） |
| 安全性 | 较高（可加密传输） | 相对较低（暴露于公网） |
| 网络依赖 | 可支持离线操作 | 强依赖网络 |
| 用户体验 | 更丰富、流畅 | 受浏览器限制 |
...
IP地址是设备在网络中的唯一标识，用于定位网络中的主机。

| 版本        | 格式 | 位数 | 示例 | 扩展                       |
|-----------|------|------|------|--------------------------|
| IPv4(已用完) | 点分十进制 | 32位 | `192.168.1.1` | 分为公网IP和局域网IP（以192.168开头） |
| IPv6      | 冒号十六进制 | 128位 | `2001:0db8:85a3::8a2e:0370:7334` |                          |


:::tip 扩展
- 127.0.0.1 — 本地回环地址（localhost），永远指向本机IP
- 0.0.0.0 — 监听所有网络接口
- 255.255.255.255 — 广播地址
- `ip config` 查看本机IP地址；`ping ip地址`可测试与某设备是否能连通
:::
#### 端口
...
:::tip 扩展
- 端口号取值范围为`0-65535`的整数，一般人自己只可用`1024`以上端口（0-1023为知名服务专用）
- 一个端口号只能对应一个应用程序
...
| 协议 | 全称 | 特点 | 应用场景 |
|------|------|------|---------|
| TCP | 传输控制协议 | 面向连接、可靠、有序 | 文件传输、邮件、网页 |
| UDP | 用户数据报协议 | 无连接、不可靠、快速 | 视频流、游戏、DNS |
| HTTP | 超文本传输协议 | 应用层、基于TCP | 网页浏览 |
| WebSocket | 套接字协议 | 全双工通信 | 实时聊天、推送 |

详情查看：[进阶 > HTTP协议](/frontend/advanced/http.html)
