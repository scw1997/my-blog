...
这三种加载器遵循一套被称为“双亲委派”（Parent Delegation Model）的核心规则来协同工作。

1. 当一个类加载器（比如应用程序类加载器）收到加载某个类的请求时，它不会立刻自己动手。 
2. 它会先将这个请求委派给自己的父加载器（扩展类加载器）。
3. 扩展类加载器收到请求后，同样不会自己加载，而是继续向上委派给它的父加载器（启动类加载器）。
...
- 发生区域：元空间（方法区），用于存储类的元数据（如类结构、方法信息等）。在JDK 8之前，这个区域被称为`永久代`（PermGen）。
...
:::tip 新生代和老年代
JVM 根据对象存活时间的长短，把堆内存划分成了两部分：新生代和老年代。
新生代：专门用来存放新创建的对象。
老年代：存放那些在新生代中经历过多次垃圾回收后，依然存活的“老顽固”对象。
:::
