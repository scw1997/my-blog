...
:::tip js中new操作发生了什么：
- **创建一个新的空对象**：首先，JavaScript 引擎会创建一个新的空对象（即 {}）。
- **设置原型**：然后，这个新创建的对象的内部 [[Prototype]]（即 __proto__）被赋值为构造函数的 prototype 属性。这一步是原型链继承的关键，它确保了新创建的对象能够访问构造函数原型上的属性和方法。
- **改变 this 指向**：构造函数内部的 this 被绑定到新创建的对象上。因此，在构造函数内部使用 this 关键字时，它指向的是新创建的对象。
- **执行构造函数**：接下来，构造函数被调用，传入的参数会传递给构造函数（如果有的话）。如果构造函数返回了一个对象，那么这个对象会替换掉前面步骤中创建的对象；否则，返回前面步骤中创建的对象。注意，大多数构造函数不返回对象，而是初始化 this 指向的对象。
- **返回新对象**：如果构造函数没有显式返回一个对象，则返回前面步骤中创建并经过初始化的新对象。
:::
